...
import { EMPTY_AFFORDANCE } from '../src/game/affordances';
...
import { counterStepScenario, fullBoardScenario } from './corpus';
...
describe('reduceUiMode transitions', () => {
  ...
  it('idle + playable hand card requiring trash enters choosingTrash', () => {
    ...
    const result = reduceUiMode(IDLE, { kind: 'clickHandCard', instanceId: 'h1' }, a);
    expect(result.mode).toEqual({ kind: 'choosingTrash', cardToPlay: 'h1' });
  ...
  it('idle + counterable hand card enters countering', () => {
    ...
    const result = reduceUiMode(IDLE, { kind: 'clickHandCard', instanceId: 'h1' }, a);
    expect(result.mode).toEqual({ kind: 'countering', counterCard: 'h1' });
    expect(result.intent).toBeUndefined();
  });
  ...
  it('cardSelected + chooseAction routes to play intent, choosingTrash, or attacking', () => {
    ...
    expect(attack.mode).toEqual({ kind: 'attacking', attacker: 'x1' });
  ...
  it('idle + own attacker enters attacking; valid target emits DECLARE_ATTACK', () => {
    ...
    expect(enter.mode).toEqual({ kind: 'attacking', attacker: 'c1' });
  ...
  it('attacking + non-target click is a no-op', () => {
    ...
    const mode: UiMode = { kind: 'attacking', attacker: 'c1' };
  ...
  it('DON area click enters attachingDon only when some card can receive DON', () => {
    ...
    expect(reduceUiMode(IDLE, { kind: 'clickDonArea' }, yes).mode).toEqual({ kind: 'attachingDon' });
    const no = aff({});
  ...
  it('attachingDon + receivable own card emits ATTACH_DON count 1', () => {
    ...
    const mode: UiMode = { kind: 'attachingDon' };
  ...
  it('choosingTrash + candidate click emits PLAY_CARD with trashCharacter', () => {
    ...
    const mode: UiMode = { kind: 'choosingTrash', cardToPlay: 'h1' };
  ...
  it('countering + valid own target emits PLAY_COUNTER', () => {
    ...
    const mode: UiMode = { kind: 'countering', counterCard: 'h1' };
  ...
  it('escape and clickEmpty reset every mode to idle with no intent', () => {
    ...
    const modes: UiMode[] = [
      { kind: 'idle' },
      { kind: 'cardSelected', card: 'h1' },
      { kind: 'attacking', attacker: 'c1' },
      { kind: 'attachingDon' },
      { kind: 'choosingTrash', cardToPlay: 'h1' },
      { kind: 'countering', counterCard: 'h1' },
    ];
...
describe('ensureModeValid', () => {
  it('keeps a mode whose card still carries the affordance', () => {
    ...
    const mode: UiMode = { kind: 'choosingTrash', cardToPlay: playable };
    ...
    const counterMode: UiMode = { kind: 'countering', counterCard };
  ...
  it('resets attacking mode when the attacker no longer can attack', () => {
    ...
    const mode: UiMode = { kind: 'attacking', attacker };
...
describe('clickStateOf', () => {
  ...
  it('marks only the selected card in cardSelected mode', () => {
    ...
    const mode: UiMode = { kind: 'cardSelected', card: 'x1' };
